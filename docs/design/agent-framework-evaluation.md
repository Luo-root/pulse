...
2. **Go 生态在 agent 框架评测上整体缺席**（§4）——先建立 Go 内评测口径的框架，填补该生态的评测空白。
...
横评与基准缺失之外，从各项目截至 2026-09 的公开文档看，Go 框架（含 Eino）与 Python 主流阵营普遍**没有把记忆作为带治理语义的持久层来宣传与文档化**：记忆能力多表述为「历史消息列表 + 摘要」，而事件溯源（event-sourced 日志）、Supersede/Revoke 生命周期、taint 传播、审批面（HITL）这套治理语义在公开材料中少见。此处陈述的是文档与公开材料的覆盖面，不构成对各项目内部能力的完整裁决。Pulse P2 记忆层正是按这套语义建的（§17 调研对齐 Letta/Mem0/Zep 后确认这是行业前沿而非标配）。

Pulse 评测的差异化锚点由此确定：

1. **基建 benchmark**：Go 内战（Go 框架同口径对比）+ 跨语言参照，填补 Go 生态评测空白；
2. **工程能力 property tests**：崩溃恢复、token 效率、记忆投毒防护——本仓库定义的工程可靠性检查表，公开可复用，任何框架（含 Pulse 自己）都可以拿来测；截至 2026-09 的公开资料未见到竞品提供同口径检查表，这一陈述以「本评测集覆盖了什么」为准，不以「对手缺什么」为准；
...
**测什么**：本仓库定义的工程可靠性不变式，逐项转化为 property test（截至 2026-09 的公开资料，未见到竞品提供同口径的检查表；本节描述 Pulse 自测面，不构成对竞品能力的完整评估）——
...
**产出**：一套公开的「agent 框架工程可靠性检查表」——任何框架都可以直接拿来测自己，测不过的项即是可复现的证据（对包括 Pulse 自己在内的所有框架）。这是把设计文档里的不变式变成行业口径的动作。

**成本**：CI 内跑完，无 API 开销（纯进程内断言，零外部依赖）。

### 5.3 第三步：能力基准（GAIA / tau-bench）

**状态：未执行**（截至 2026-09）。启用条件：真实模型凭据到位 + 前两步的可信度建立 + 任务集与成本预算选定。在执行之前，本节是规划而非结果，不参与任何能力结论。

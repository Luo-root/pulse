...
builtins P1（Issue #58）：`web_fetch` / `web_search` / `question`。`web_fetch` GET 后抽文本再按行 `offset`/`limit` 续读（每次续读再 GET，不是浏览器 DOM）；搜索默认可注入 `Searcher`（默认 DuckDuckGo Lite）；`question` 需 `Asker`，与 HITL 批准分家。

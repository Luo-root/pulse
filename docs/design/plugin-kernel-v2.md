...
- **作用域局部绑定（#170）**：`Provide(ctx, Key, v, kernel.Local())` 把绑定
  收在本 scope 子树——父 / 兄弟 / 其他并发请求不可见、不投递变更、不进依赖
  索引（Fiber 的 `Inject` 只看全局命名空间）；`Get` 沿父链近因优先、局部
  遮蔽全局、随 scope 销毁撤除。用于请求级数据（`observability.AttachCollector`
  是首个消费者）；全局绑定语义与 `Get` 的全局回退均不变。

...
// NewScheduleLoopNode 调度循环：监听任务完成/失败，触发重规划
...
3. 严格匹配以下结构，字段名/层级不可修改：
{
	"tasks": [
		{
			"id": "task_1",
			"description": "获取当前工作目录的绝对路径，作为后续文件操作的基准路径",
			"inputs": [],
			"outputs": ["current_path"],
		},
		{
			"id": "task_2",
			"description": "基于工作目录路径，列出目录下所有文件和文件夹，确认目标文件是否存在",
			"inputs": ["current_path"],
			"outputs": ["file_list"],
		}
	]
...
func extractPlan(content, goal string) (Plan, error) {
	...
	if match == "" {
		return plan, fmt.Errorf("no JSON with 'tasks' field found in response (content length: %d)", len(content))
	}
	...
	return plan, fmt.Errorf("extractPlan: JSON parse failed, content preview: %s", preview)
}
...
func extractJSONBody(s string) string {
	// 尝试从第一个 { 开始，逐字符找匹配的 }
	// 使用花括号计数平衡，找到第一个完整的 JSON 对象
	if idx := strings.Index(s, "{"); idx >= 0 {
		if obj := extractBalancedJSON(s[idx:]); obj != "" {
			return obj
		}
	}
	...
	// 数组同理
	if idx := strings.Index(s, "["); idx >= 0 {
		if arr := extractBalancedJSON(s[idx:]); arr != "" {
			return arr
		}
	}
...
// extractBalancedJSON 从 s 的开头提取第一个完整的 JSON 对象/数组
// 通过花括号/方括号计数平衡，遇到第一个完整的闭合就停止
...
// parsePlanJSON 多策略解析 LLM 返回的计划 JSON
// 策略顺序：直接解析 → 清洗后解析 → 提取后解析 → 提取+清洗后解析
func parsePlanJSON(content string, plan *Plan) error {
	// Strategy 1: 直接解析
	if err := json.Unmarshal([]byte(content), plan); err == nil && len(plan.Tasks) > 0 {
		return nil
	}

	// Strategy 2: TrimJSONWrapper 后解析
	trimmed := TrimJSONWrapper(content)
	if trimmed != "" {
		if err := json.Unmarshal([]byte(trimmed), plan); err == nil && len(plan.Tasks) > 0 {
			return nil
		}

		// Strategy 3: 清洗后解析（修复尾逗号等常见问题）
		fixed := sanitizeJSON(trimmed)
		if err := json.Unmarshal([]byte(fixed), plan); err == nil && len(plan.Tasks) > 0 {
			return nil
		}
	}

	// Strategy 4: 正则提取含 "tasks" 的 JSON 块
	extracted, err := extractPlan(content, plan.Goal)
	if err != nil {
		return err
	}
...
// sanitizeJSON 修复 LLM 输出的常见 JSON 问题

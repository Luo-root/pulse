...
!!! 口径：i9-14900HX / Go 1.25 / Windows，`-count=5`（war 表为 `-count=2`）。**比量级与倍数区间，不比个位数**（轮次间方差显著）；**分配数跨运行稳定，是可依赖的一等证据，绝对 ns 不是**。
...
2. **编排 fan-out 免费**：flow 的 AND 槽位让分支汇聚几乎不加价——DAG（T4）与线性链（T3）同价同 allocs；对比方的 join 调度较其自身线性链贵 ~1.7×，Workflow 字段映射再 +12–17%。

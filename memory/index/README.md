...
- **Recheck errors are reported by class**: when a hit is re-read from the store for verification, `ErrItemNotFound` counts as stale and is skipped (the index is ahead of the store; the derived index is disposable); `ctx` cancellation / timeout is reported as-is; any other store failure is reported — errors on the recheck path are never swallowed silently.

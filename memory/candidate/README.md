...
- **Explainability**: the `Report{Extracted, Stored, Duplicates, Invalid}` counters — silent drops are forbidden; `DuplicateHits` corresponds one-to-one with `Duplicates` (except the ID-collision defensive count, which has no existing item to point at) and reports the ID and status of the existing item that matched (including the ones still blocked under the conservative v1 rule when Revoked / Superseded); a dedup query failure aborts the batch (on store failure, prefer failing and letting the host retry).

...
- `ExportItems` exports everything: **`IncludeInactive` is forced** — Superseded/Revoked are part of the state machine too; dropping them means dropping the supersede chain and revocation history. **`Limit` is ignored** — an export is all-or-nothing (a query-side page parameter silently truncating a cold backup is worse than an error; use `Search` when you want paging).

...
| `candidate` | **已落地**（P2-D3，Issue #90） | 候选记忆管线：extractor seam（宿主 LLM 提取，模型参数最小化）→ 去重（归一包含式保守口径，内存双归一——存量/候选两侧同口径，含批次内）→ Pending 入库（检索天然不可见）→ approve=Supersede 晋升（SourceRefs 追加 manual 审批标记）/ reject=Revoke；审批作用域=namespace 完全相等（越界 ErrOutsideScope）；taint 默认 untrusted-external（ASI06），批准不改 taint；`Metrics()` 累计快照（D4 指标面） |
| `reflection` | **已落地**（P2-D4，Issue #92，P2 收口） | 可配置 background reflection（§10.3）：输入预算截断（尾部保留整条消息，rune 口径同 CharMeter）→ candidate 提炼 → 计数；无后台循环默认关（触发时机归宿主）；输出只到候选（审批人盖章）；`Metrics()` = token 成本运行计数（真实 usage 归宿主桥） |
...
memory/reflection → memory/candidate + llm
```

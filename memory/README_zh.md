...
| `candidate` | **已落地**（P2-D3，Issue #90） | 候选记忆管线：extractor seam（宿主 LLM 提取，模型参数最小化）→ 去重（归一包含式保守口径，内存双归一——存量/候选两侧同口径）→ Pending 入库（检索天然不可见）→ approve=Supersede 晋升（SourceRefs 追加 manual 审批标记）/ reject=Revoke；审批作用域=namespace 完全相等（越界 ErrOutsideScope）；taint 默认 untrusted-external（ASI06），批准不改 taint |

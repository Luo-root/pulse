...
**Model-supplied arguments are not guaranteed to be valid JSON**: adapters pass the provider's tool-call argument string through verbatim (only an empty string becomes `{}`), while `json.RawMessage`'s `MarshalJSON` fails hard on invalid JSON — fail-closed persistence would promote one bad argument into a failed round, cutting off the path the tool layer already tolerates (the tool reports a plain argument error as `IsError` and the model retries). So host applies a **lossless fallback** before persisting: an invalid argument is encoded as a JSON **string** holding its raw text (read it back with `json.Unmarshal` into a `string`; the one exception is raw text containing invalid UTF-8 bytes, which JSON text cannot carry — those are replaced per spec by U+FFFD), while **the tool and later requests still receive the raw text** — the fallback touches only the persisted copy, and valid arguments pass through byte for byte. The price is exactly one thing: in the log that row's `arguments` drifts from an object to a string, so a request rebuilt from the log cannot restore the object shape. `tool.called` keeps dropping malformed arguments (its codec accepts valid JSON only); `message.assistant` is the authority for the arguments.

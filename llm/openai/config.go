...
// Provider 名：Registry.RegisterProvider 的登记键。
const (
	ProviderCompletions = "openai"           // Chat Completions 线协议
	ProviderResponses   = "openai-responses" // Responses 线协议
)

// Register 把两个 OpenAI 变体登记到 reg。每次 RegisterProvider 都是
// 独立的可逆效应：adapter 插件在自己的 Apply 中调用并丢弃返回值即可，
// 插件卸载时工厂与其打开的实例随作用域自动收回。
func Register(scope *kernel.Context, reg *llm.Registry) error {
	if _, err := reg.RegisterProvider(scope, ProviderCompletions, NewCompletions); err != nil {
		return err
	}
	_, err := reg.RegisterProvider(scope, ProviderResponses, NewResponses)
	return err
}

// adapterOptions 是 Config.Options 中本包认识的键。Options 同时承载
// 多个 adapter 的参数，按名取用、未知键忽略。
type adapterOptions struct {
	// Organization / Project 映射 OpenAI-Organization / OpenAI-Project 头。
...
// newClient 构造两个变体共享的 SDK 客户端：认证、端点、重试策略、
// 附加头。APIKey 必填——本包不读环境变量，配置显式优先。
func newClient(cfg llm.Config) (sdk.Client, error) {
	...
	if opts.Organization != "" {
		o = append(o, option.WithOrganization(opts.Organization))
	}
	if opts.Project != "" {
		o = append(o, option.WithProject(opts.Project))
	}
...
// mapError 把 SDK/传输层错误统一翻译为 *llm.Error。SDK 的 API 错误
// 携带状态码与错误体；其余按传输层语义分类。
func mapError(provider string, err error) error {
	...
	if errors.As(err, &apiErr) {
		kind := classifyStatus(apiErr.StatusCode, apiErr.Code, apiErr.Message)
		return llm.NewError(kind, provider, apiErr.StatusCode, err, "%s", apiErr.Message)
	}
...
// classifyStatus 按 HTTP 状态码与错误体分类。上下文超长与内容过滤
// 在 OpenAI 侧以 400 形态出现，靠错误码/消息二次识别。
func classifyStatus(status int, errCode, msg string) llm.ErrKind {
	code := strings.ToLower(errCode)
	text := strings.ToLower(msg)
	switch {
	case status == 401 || status == 403:
		return llm.ErrAuth
	case status == 429 || strings.Contains(code, "rate_limit"):
...
// unsupportedPart 构造"词汇表块不被该变体支持"的统一错误——
// 显式失败，不静默丢弃内容。

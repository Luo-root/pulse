...
渐进示例的第三层：验证 `kernel/flow` 在「一次用户请求」上到底承担了什么（并行 / Skip 分支见 [04-flow-dag](../04-flow-dag/README.md)）。**先说结论：flow 在这里不是装饰，它把请求内的数据依赖、失败语义和可观测性从过程式代码变成声明式契约；但对话历史被有意排除在 flow 之外。**

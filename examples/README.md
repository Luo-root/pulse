...
| 07 | [`07-production`](07-production/) | Production integration: MCP/Skills multi-source, observability adapters, reflection & metrics | all | 03 04 06 |
...
07      + MCP / Skills multi-source + observability adapters + metrics  ← shippable
```
...
- **Runtime**: per-package adapters (`llm.Observe` / `loop.Observe` / `flow.NewRecordObserver`) fold llm/loop/flow facts into the same Sink (`source=bridge`, `trace_id` required) — lesson 02 hand-writes the three official wiring calls plus Collector direct-write; from lesson 03 on, lessons reuse the packaged `demoapp.Host.NewObserve`.
...
- `examples/internal/demoapp` is lesson-private assembly scaffolding (REPL shell, `.env` loading, packaged observability wiring/HITL); **the library packages themselves have no internal** — this does not violate the "no internal in library packages" convention.
- Teaching rhythm: lessons 01–03 hand-write the key implementations (assembly chain / observability wiring / HITL) in-lesson, with the demoapp packaged versions as their reference prototypes; from lesson 04 on, lessons reuse the packaged versions, and every `main.go` stays deliberately "readable in one pass" while complexity settles into demoapp or the corresponding package.

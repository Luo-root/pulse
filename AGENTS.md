...
memory/                     # P2 记忆与会话（设计 Accepted：docs/design/memory-layer-research-and-v2-design.md）；session/ 已落地（P2-A in-memory+JSONL/blobs/文件锁、P2-B Replace fold），compaction/ 已落地（P2-B meter+事务编排+pruning），store/ 已落地（P2-C1 内存 item store：namespace 隔离+Supersede/Revoke+CAS），assemble/index 未建
